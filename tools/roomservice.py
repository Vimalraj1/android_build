...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from OneRomOne Github (http://github.com/OneRomOne)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:OneRomOne+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('OneRomOne/%s already fetched to %s' % (repo_name, repo_target))
            continue
...
print("Repository for %s not found in the OneRomOne Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
